triangle <- filter(full_data, key == "25275_ppauto")

stan_file = # stan file with best porameters
stan_file_niwp = # stan file with best porameters

results_am <- get_estimates(triangle,
                            stan_file = stan_file,
                            iter = 5000,
                            type = "list")

results_lr <- get_estimates_lr(triangle,
                               stan_file = stan_file,
                               iter = 5000,
                               type = "list")

results_am_niw <- get_estimates(triangle,
                            stan_file = stan_file_niwp,
                            iter = 5000,
                            type = "list")

results_lr_niw <- get_estimates_lr(triangle,
                               stan_file = stan_file_niwp,
                               iter = 5000,
                               type = "list")
