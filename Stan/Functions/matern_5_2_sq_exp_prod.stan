functions {
matrix cov_matern_5_2_sq_exp_prod(vector x1, vector x2, real alpha, real rho,
    ...
    for (i in 1:(N - 1)) {
      ...
      for (j in (i + 1):N) {
        ...
        K[i, j] = alpha*(1+sqrt(5*dsq)/rho+5*dsq/(3*rho^2))*exp(-sqrt(5*dsq)/rho)*exp(-dsq/(2*rho^2));
        K[j, i] = K[i, j];
      }
    }
    K[N,N] = alpha + sigma;
...
model{
  matrix[N,N] K_exp = cov_matern_5_2_sq_exp_prod(x1, x2, alpha, rho, a1, b1, a2, b2, psi1, psi2, sigma) + diag_matrix(rep_vector(1e-9, N));
