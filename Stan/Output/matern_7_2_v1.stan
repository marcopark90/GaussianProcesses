functions {
matrix cov_matern_7_2(vector x1, vector x2, real alpha, real rho,
    ...
    for (i in 1:(N - 1)) {
      ...
      for (j in (i + 1):N) {
        ...
        K[i, j] = alpha*(1+sqrt(7*dsq)/rho+14*dsq/(5*rho^2)+(7*sqrt(7)*sqrt(dsq)*dsq)/(15*rho^3))*exp(-sqrt(7*dsq)/rho);
        K[j, i] = K[i, j];
      }
    }
    K[N,N] = alpha + sigma;
...
model{
  matrix[N,N] K_exp = cov_matern_7_2(x1, x2, alpha, rho, a1, b1, a2, b2, psi1, psi2, sigma) + diag_matrix(rep_vector(1e-9, N));
